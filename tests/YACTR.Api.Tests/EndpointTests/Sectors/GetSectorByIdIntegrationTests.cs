...
public class GetSectorByIdIntegrationTests(ApiTestClassFixture fixture) : TestBase<ApiTestClassFixture>
{
    ...
    public async Task GetById_WithValidId_ReturnsSector()
    {
        ...
        var (response, result) = await client.GETAsync<GetSectorById, GetSectorByIdRequest, SectorResponse>(new(sector.Id));
    ...
    {
        using var client = fixture.CreateAuthenticatedClient();
        var (response, _) = await client.GETAsync<GetSectorById, GetSectorByIdRequest, SectorResponse>(new(Guid.NewGuid()));
    ...
    public async Task GetSectorById_WithSectorImages_ReturnsSectorWithImageUrls()
    {
        ...
        var (response, result) = await client.GETAsync<GetSectorById, GetSectorByIdRequest, SectorResponse>(new(createdSector.Id));
        response.IsSuccessStatusCode.ShouldBeTrue();
        result.ShouldNotBeNull();
